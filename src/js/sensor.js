 ...
 * @property {number} rayCount - The number of rays to cast (default: 3).
 * @property {number} rayLength - The length of each ray (default: 150).
 * @property {number} raySpread - The angle spread of the rays (default: Math.PI / 2).
 * @property {Array} rays - Array of rays, each represented by two points (start and end).
 * @property {Array} readings - Array to hold the readings from the rays (not used in this implementation).
 *
 * @method update - Updates the rays based on the car's position and angle.
 * @method #castRays - Casts rays from the car's position at specified angles.
 * @method draw - Draws the rays on the canvas.
 */
class Sensor {
  constructor(car) {
    this.car = car;
    ...
    this.raySpread = Math.PI / 4; // Spread angle of the rays
  ...
  /**
   * Finds the closest intersection point for a specific ray with road borders.
   *
  ...
  /**
   * Draws the rays on the canvas.
   * This method iterates through the rays array and draws each ray on the canvas using the
